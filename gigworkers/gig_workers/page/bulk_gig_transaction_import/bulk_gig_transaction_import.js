frappe.pages["bulk-gig-transaction-import"].on_page_load = function (wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: "Bulk Gig Transaction Import",
		single_column: true,
	});
	new GigTransactionBulkImport(page);
};

class GigTransactionBulkImport {
	constructor(page) {
		this.page       = page;
		this.import_id  = null;
	...
	_render() {
		this.page.main.html(`
			<div class="gt-import-wrap" style="max-width:780px; margin:24px auto; padding:0 16px;">

				<div class="card mb-4" style="border-left:4px solid #1a73e8;">
					<div class="card-body">
						<h5 class="card-title" style="color:#1a73e8;">High-Volume Gig Transaction Import</h5>
						<p class="card-text text-muted mb-2">
							Import <strong>more than 1 lakh transactions</strong> in background batches.
							OTP emails, welfare payment creation, and duplicate notifications are
							skipped during import for maximum speed.
						</p>
						<ol class="mb-0 text-muted" style="font-size:13px; padding-left:18px;">
							<li>Download the CSV template and fill in your transaction data.</li>
				...
				<div class="mb-4" style="display:flex; gap:10px; align-items:center;">
					...
					<span id="gt-btn-hint" class="text-muted" style="font-size:12px;">
						Upload a file first to begin.
					</span>
				</div>

				<div id="gt-progress-section" style="display:none;">
					<hr>
					<h6>Import Progress</h6>
					<div class="progress mb-2" style="height:22px;">
						<div id="gt-progress-bar" class="progress-bar progress-bar-striped active"
							style="width:0%; transition:width .4s;">0%</div>
					</div>
					...
					<div id="gt-error-box" style="display:none;">
						<label class="control-label text-danger">Validation Errors / Skipped Rows</label>
						<textarea id="gt-error-log" class="form-control" rows="8" readonly
	...
	_bind_events() {
		...
		const area  = this.page.main.find("#gt-file-area");
		const input = this.page.main.find("#gt-file-input");

		[area, input].forEach((el) => {
			el.on("dragover", (e) => { e.preventDefault(); area.css("border-color", "#1a73e8"); });
		...
		frappe.realtime.on("gt_bulk_import_done", (data) => {
	...
	_handle_file(file) {
		...
		if (!ok.includes(file.type) && !file.name.match(/\.(csv|xlsx|xls)$/i)) {
			frappe.msgprint(__("Please upload a CSV or XLSX file.")); return;
		}
		this.page.main.find("#gt-file-area").css("border-color", "#ccc");
		this.page.main.find("#gt-file-label").html(
			`<strong>${file.name}</strong> &nbsp;(${this._human_size(file.size)})`
		);
		this.page.main.find("#gt-upload-status").text("Uploading…").css("color", "#888");
	...
	_upload_file(file) {
		...
		xhr.onload = () => {
			...
			if (xhr.status === 200 && res.message && res.message.file_url) {
				...
				this.page.main.find("#gt-file-area").css("border-color", "#27ae60");
				this.page.main.find("#gt-upload-status").text("File ready ✓").css("color", "#27ae60");
				this.page.main.find("#gt-btn-hint").text("File uploaded. Click Start Import.");
				btn.prop("disabled", false).html('<i class="fa fa-play"></i>&nbsp; Start Import');
			} else {
				const msg = res.exc_type || res.message || xhr.statusText || "Unknown error";
				this.page.main.find("#gt-upload-status").text("Upload failed: " + msg).css("color", "#e74c3c");
				this.page.main.find("#gt-file-area").css("border-color", "#e74c3c");
				btn.prop("disabled", false).html('<i class="fa fa-play"></i>&nbsp; Start Import');
				frappe.msgprint(__("File upload failed: ") + msg);
			}
		};
		xhr.onerror = () => {
			this.page.main.find("#gt-upload-status").text("Network error.").css("color", "#e74c3c");
			btn.prop("disabled", false).html('<i class="fa fa-play"></i>&nbsp; Start Import');
		};
		xhr.send(formData);
	}
	...
	_start_import() {
		if (!this.file_url) {
			frappe.msgprint({ title: __("No File Selected"),
		...
		frappe.call({
			...
			callback: (r) => {
				if (r.message && r.message.import_id) {
					...
					this._start_polling();
					frappe.show_alert({ message: __("Import job started in background."), indicator: "blue" }, 5);
				} else {
					this.page.main.find("#btn-start-import").prop("disabled", false)
	...
	_cancel_import() {
		...
		frappe.call({
			method: "gigworkers.gig_workers.page.bulk_gig_transaction_import.bulk_gig_transaction_import.cancel_import",
			args: { import_id: this.import_id },
			callback: () => frappe.show_alert({
				message: __("Cancel signal sent. Import stops after the current batch."),
				indicator: "orange" }, 6),
		});
	}

	_show_progress_section() {
		this.page.main.find("#gt-progress-section").show();
		this.page.main.find("#btn-start-import").hide();
		this.page.main.find("#gt-btn-hint").hide();
	...
	_stop_polling() {
		...
		this.page.main.find("#btn-start-import").show().prop("disabled", false)
			.html('<i class="fa fa-play"></i>&nbsp; Start Another Import');
		this.page.main.find("#gt-btn-hint").show().text("Upload a new file to import again.");
		this.file_url = null;
	}
	...
	_refresh_progress() {
		if (!this.import_id) return;
		frappe.call({
			method: "gigworkers.gig_workers.page.bulk_gig_transaction_import.bulk_gig_transaction_import.get_import_progress",
			args: { import_id: this.import_id },
			callback: (r) => {
				if (!r.message) return;
				this._apply_stats(r.message);
				if (r.message.errors && r.message.errors.length) {
					this.page.main.find("#gt-error-box").show();
					this.page.main.find("#gt-error-log").val(r.message.errors.join("\n"));
				}
	...
	_apply_stats(d) {
		...
		const bar = this.page.main.find("#gt-progress-bar");
		bar.css("width", pct + "%").text(pct + "%");
		if (status === "Completed") bar.removeClass("active").css("background-color", "#27ae60");
		else if (status === "Failed") bar.removeClass("active").css("background-color", "#e74c3c");
		else if (status === "Cancelled") bar.removeClass("active").css("background-color", "#e67e22");

		this.page.main.find("#stat-status").text(status);
		this.page.main.find("#stat-total").text(
			total > 0 ? this._fmt(total) : (status === "Running" ? "Loading…" : "—")
		);
	...
	_load_import_logs(reset = true) {
		const tbody      = this.page.main.find("#gt-log-tbody");
		const loadMoreBtn = this.page.main.find("#btn-load-more-logs");
		const countEl    = this.page.main.find("#gt-log-count");
		...
		frappe.call({
			method: "gigworkers.gig_workers.page.bulk_gig_transaction_import.bulk_gig_transaction_import.get_import_logs",
			...
			callback: (r) => {
				...
				const rows = logs.map((log) => {
					...
					const badge = `<span style="color:${color};">●</span> <span style="font-weight:600;">${log.status || "—"}</span>`;
					const dt    = log.import_date ? frappe.datetime.str_to_user(log.import_date) : "—";
					const errSty = (log.error_count || 0) > 0 ? "color:#e74c3c; font-weight:600;" : "";
					const insSty = (log.inserted || 0) > 0 ? "color:#27ae60; font-weight:600;" : "";
					return `<tr>
						...
						<td style="text-align:right; ${insSty}">${this._fmt(log.inserted)}</td>
						<td style="text-align:right; color:#e67e22;">${this._fmt(log.skipped)}</td>
						<td style="text-align:right; ${errSty}">${this._fmt(log.error_count)}</td>
	...
	_show_log_detail(log_name) {
		frappe.call({
			method: "gigworkers.gig_workers.page.bulk_gig_transaction_import.bulk_gig_transaction_import.get_log_detail",
			args: { log_name },
			freeze: true,
