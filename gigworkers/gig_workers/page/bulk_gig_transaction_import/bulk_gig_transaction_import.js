...
class GigTransactionBulkImport {
	...
	_bind_events() {
		...
		this.page.main.find("#btn-view-full-log").on("click",   () => {
			frappe.set_route("List", "Gig Transaction Import Log");
		});
		...
		frappe.call({
			...
			callback: (r) => {
				...
				tbody.html(logs.map((log) => {
					...
					const link   = `<a href="/app/gig-transaction-import-log/${log.name}" target="_blank">${log.name}</a>`;
					const errSty = log.error_count > 0 ? "color:#e74c3c; font-weight:600;" : "";
					return `<tr>
						<td>${link}</td>
						<td>${dt}</td>
